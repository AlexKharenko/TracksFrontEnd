<template>
  <teleport to="#pop-up" v-if="isUpdateGenrePopActive">
    <div class="pop-up">
      <div class="pop-up-content-container">
        <button
          class="btn close-pop-btn btn-margin"
          @click="changeUpdateGenrePopUpActivity(false)"
        >
          Close
        </button>
...
export default {
  ...
  computed: {
    ...mapGetters(["isUpdateGenrePopActive"]),
  },
  methods: {
    ...mapActions("uploading", ["changeDataUploadStatus"]),
    ...mapActions(["changeUpdateGenrePopUpActivity"]),
    ...
    async handleSubmit() {
      ...
      if (success) {
        this.changeUpdateGenrePopUpActivity(false);
      } else {
        this.error = message;
      }
    },
  ...
  watch: {
    async isUpdateGenrePopActive() {
      if (!this.isUpdateGenrePopActive) this.error = "";
      if (this.isUpdateGenrePopActive) {
        this.changeDataUploadStatus(false);
        await this.genre_id;
        const { genre_name } = await this.$store.dispatch("fetchGenreById", {
          id: this.genre_id,
        });
